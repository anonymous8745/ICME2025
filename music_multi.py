...
class MusicTransformerMulti(torch.nn.Module):
    def __init__(self, embedding_dim=None, d_inner=None, d_condition=None, vocab_size=None, num_layer=None, num_head=None,
        ...
        self.emotion_quadrant = torch.nn.Linear(4, self.d_condition) # encode quadrant emotions

        self.emotion_continuous = torch.nn.Linear(1, self.d_condition) # encode continuous emotions

        if self.d_condition > 0:
            # self.fc_condition = torch.nn.Linear(2, self.d_condition)
            self.fc_condition = 0.5 * self.emotion_quadrant + 0.5 * self.emotion_continuous # weighted sum of quadrant and continuous emotions
    ...
    def forward(self, x, condition):
        ...
        if self.d_condition > 0:
            ...
            x = torch.nn.MultiheadAttention(condition, x, x) # attention fusion of emotion embeddings and music embeddings
